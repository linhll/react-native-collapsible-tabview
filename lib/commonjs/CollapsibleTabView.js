...
const CollapsibleTabView = ({
  ...
  const prevOffset = _react.default.useRef(0);
  ...
  const onLayout = () => {
    ...
    if (currentRef) {
      const offset = disableSnap ? Math.min(prevOffset.current, collapsedHeight) : prevOffset.current >= collapsedHeight * snapThreshold ? collapsedHeight : 0;
      (0, _scrollScene.default)({
